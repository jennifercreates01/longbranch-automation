...
app.post("/api/facilities/:id/jobs", async (req, res) => {
    try {
        ...
        const { jobNumber, name, description, status, startDate, endDate, } = req.body;
        if (!jobNumber?.trim()) {
            return res.status(400).json({
...
app.put("/api/jobs/:id", async (req, res) => {
    try {
        const id = Number(req.params.id);
        const { jobNumber, name, description, status, startDate, endDate, facilityId, } = req.body;
...
app.post("/api/invoices", async (req, res) => {
    try {
        const { invoiceNumber, customerId, jobId, issueDate, dueDate, discount = 0, notes, lineItems, } = req.body;
